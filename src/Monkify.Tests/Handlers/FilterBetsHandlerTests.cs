...
namespace Monkify.Tests.Handlers
{
    public class FilterBetsHandlerTests : UnitTestsClass
    {
        ...
        public async Task FilterBets_NoPublicBets_ShouldReturnSuccess()
        {
            ...
            var session = new Session();
            session.Status = SessionStatus.WaitingBets;
            session.Parameters = new SessionParameters() { Name = Faker.Random.Word(), Description = Faker.Random.Words(6), AcceptDuplicatedCharacters = true, ChoiceRequiredLength = 4, RequiredAmount = 2, AllowedCharacters = SessionCharacterType.Letters, PresetChoices = presetChoices };
            session.Bets = new List<Bet>()
            {
